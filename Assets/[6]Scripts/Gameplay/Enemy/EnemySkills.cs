...
public class EnemySkills : EnemySkillBase
{
    ...
    public override void CastSkill(int phase, System.Action onFinished)
    {
        ...
        {
            // 2페이즈 로직 (기존 동일)
            do { rnd = Random.Range(0, 5); } while ((rnd + 100) == lastSkillId);
            lastSkillId = rnd + 100;
